...
#error "Can't use decode_dtrace module if GEP option -gen-with-trace is not activated"
#endif
...
#ifndef CACHE_DEPTH
#define CACHE_DEPTH 8     /* Must be greater or equal than 2 */
#endif
#ifndef CACHE_SIZE
...
$(proc)_decoder_t *$(proc)_new_decoder($(proc)_platform_t *pf)
{
    ...
    if (res == NULL)
                $(proc)_error("not enough memory to create a $(proc)_decoder_t object");
...
{	/* Is the instruction inside the cache ? */
	...
	/* If it's last element LRU can be simplified */
	current->next = init;
...
$(proc)_inst_t *$(proc)_decode($(proc)_decoder_t *decoder, $(proc)_address_t address)
{
	...
	{
		$(proc)_address_t a = address + (i << 2);
		id = $(proc)_fetch(decoder->fetch, a, &code);
		is_branch = $(proc)_decode_table[id](code, (res+i));
		(res+i)->addr = a;
	}
	res[i].ident = -1;
	return res;
