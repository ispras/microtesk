...
class InstructionBlock
  ...
  attr_accessor :name, :items, :attributes
  ...
  # Block construction
  def build(j_block_builder_factory)
    ...
    @items.each do |item|
      ...
      if item.is_a? Label
        if delayed_instruction == nil
          ...
          delayed_instruction.add_item_to_attribute "f_labels", item.java_object
          puts "Label " + item.to_s
        end
      else
        if delayed_instruction != nil
          delayed_labels.each do |i_item|
            delayed_instruction.add_item_to_attribute "b_labels", i_item.java_object
      ...
      if delayed_instruction != nil
        j_block_builder.addCall delayed_instruction.build(j_block_builder_factory.newAbstractCallBuilder(delayed_instruction.name))
        delayed_instruction = nil
      end

      if item.is_a? InstructionBlock
        j_block_builder.addBlock(item.build j_block_builder_factory)
        delayed_instruction = nil
      end
...
class Instruction
  ...
  def build(j_instruction_builder)
    ...
    @arguments.each_pair do |name, value|
      if value.is_a? String or value.is_a? Symbol
        ...
        l = Java::Ru.ispras.microtesk.test.template.Label.new(value.to_s, @block_id)
        
        @attributes["labels"].push [l, name]
