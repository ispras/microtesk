...
public final class Aspectracer {
  ...
  private String create() throws IOException {
    close();
    final String fileName = String.format("%s_%04d.%s", filePrefix, fileCount++, fileExtension);
  ...
  private void printToTrace(final String mark) {
    ...
    Tracer.traceMessage(new TraceMessage("coverage", new TraceNode("element", "aspect", "coverage",
        "name", buf.toString(), "cs", "instruction paths")));
  }
}
