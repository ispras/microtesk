...
public class ArithmMod implements IBinaryOperator {
...
// , ETypeID.FIX // NOT SUPPORTED IN THIS VERSION
  ...
  @Override
  public Data execute(final Data left, final Data right) {
    assert left.getType().equals(right.getType()) : "Restriction: types (and sizes) should match.";
    final Type resultType = left.getType();

    final BitVector result = resultType.getTypeId() == TypeId.INT ?
        BitVectorMath.srem(left.getRawData(), right.getRawData()) :
        BitVectorMath.urem(left.getRawData(), right.getRawData());

    return new Data(result, resultType);
  }

  @Override
  public boolean supports(final Type left, final Type right) {
