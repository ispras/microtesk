...
final class Float32Operations implements Operations {
  ...
  public FloatX round(final FloatX value) {
    final float result = JSoftFloat.float32_round_to_int(value.floatValue());
    return newFloatX(result);
  }
  ...
  public FloatX toFloat(final FloatX value, final Precision precision) {
    switch (precision) {
      case FLOAT16: {
        final Float16 float16Value = Float16Functions.f32_to_f16(value.floatValue());
        return Float16Operations.newFloatX(float16Value);
      }
