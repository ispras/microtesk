...
public class MmuExecutionClassifierDevice extends MmuExecutionClassifier {
  @Override
  public List<MmuExecutionClass> unifyExecutions(final List<MmuExecution> executions) {
    final List<MmuExecutionClass> executionsOfOperation = new ArrayList<>();

    for (MemoryOperation operation : MemoryOperation.values()) {
      // For Store and Load
      final Map<Map<MmuDevice, BufferAccessEvent>, MmuExecutionClass> mapExecution =
          new LinkedHashMap<>();
      ...
      for (final MmuExecution execution : executions) {
        if (execution.getOperation() == operation) {
          ...
          if (mapExecution.containsKey(mapDevices)) {
            final MmuExecutionClass mmuExecutionClass = mapExecution.get(mapDevices);
  ...
  private Map<MmuDevice, BufferAccessEvent> getDevicesAndEvents(
    ...
    Map<MmuDevice, BufferAccessEvent> result = new HashMap<>();
