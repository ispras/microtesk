...
public class EndlessLoopTestCase extends MiniMipsTest {
  @Test
  public void test() {
    final Statistics statistics = run("endless_loop.rb");
  ...
  protected boolean isExpectedError(final String message) {
    return super.isExpectedError(message)
        || message.contains("reached its limit on execution count");
  }
}
